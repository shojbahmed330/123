...
export class AIController {
  ...
  constructor(
    ...
    try {
      ...
      this.geminiService = gemini || new GeminiService();
      this.openRouterService = new OpenRouterService();
      this.orchestrator = orchestrator || new Orchestrator(this.diffEngine, this.geminiService);
    } catch (error) {
  ...
  async *processRequest(
    ...
    const isOpenRouter = modelName.includes('/') || modelName.startsWith('gpt') || modelName.startsWith('claude');
    const provider = isOpenRouter ? this.openRouterService : this.geminiService;
