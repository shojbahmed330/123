  ...
  const handleSend: (customPrompt?: string, isAuto?: boolean, overrideQueue?: string[]) => Promise<void> = useCallback(async (customPrompt?: string, isAuto: boolean = false, overrideQueue?: string[]) => {
    ...
    try {
      ...
      if (projectConfig.supabase_url && projectConfig.supabase_key) {
        augmentedPrompt += `\n\n[SYSTEM CONTEXT: The user has configured Supabase in their project settings. Supabase URL: ${projectConfig.supabase_url}, Supabase Anon Key: ${projectConfig.supabase_key}. If the user requests database functionality or SQL, you MUST generate the necessary SQL for the database in a file named 'database.sql' (or appropriate migration files) so the user can run it in their Supabase project. Also, ensure the frontend code uses these credentials to connect to Supabase (e.g., via a supabaseClient file).]`;
      }
