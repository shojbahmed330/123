  ...
  try {
    ...
    const envConfig = `
      window.__STUDIO_ENV__ = {
        ...
        DEV: true,
        PROD: false,
        VITE_SUPABASE_URL: "${projectConfig?.supabase_url || ''}",
        VITE_SUPABASE_ANON_KEY: "${projectConfig?.supabase_key || ''}"
      };
    `;
    ...
    Object.entries(projectFiles).forEach(([path, content]) => {
      ...
      if (path.endsWith('.css') || path.endsWith('.scss') || path.endsWith('.sass')) {
      ...
      } else if (path.match(/\.(svg|png|jpg|jpeg|gif|webp|ico)$/i)) {
        // For binary/image files, just export a placeholder or the data URI if it's already base64
        // In this environment, content might be raw text or base64 depending on how it was loaded.
        // To prevent import crashes, we export a simple string.
        const isBase64 = content.startsWith('data:');
        const exportValue = isBase64 ? content : `data:image/svg+xml;utf8,${encodeURIComponent(content)}`;
        transpiledFiles[path] = `export default "${exportValue}";`;
        ...
        // Catch-all for any other file type (e.g. fonts, unknown assets)
        // Export an empty string to prevent import crashes
        transpiledFiles[path] = `export default "";`;
      }
    });
    ...
    if (hasReact) {
      ...
      const entryFile = files.find(f => 
        ...
        f === `${workspacePrefix}src/main` || f === `${workspacePrefix}src/index` || f === `${workspacePrefix}src/App` ||
        f === 'src/main' || f === 'src/index' || f === 'src/App' ||
        f === 'frontend/main' || f === 'frontend/index' || f === 'frontend/App' ||
        f === 'frontend/src/main' || f === 'frontend/src/index' || f === 'frontend/src/App' ||
        f === 'client/main' || f === 'client/index' || f === 'client/App' ||
        f === 'client/src/main' || f === 'client/src/index' || f === 'client/src/App' ||
        f === 'main' || f === 'index' || f === 'App'
      ) || 
      files.find(f => f.startsWith(workspacePrefix) && (f.includes('components/') || f.includes('pages/'))) ||
      files.find(f => f.startsWith('src/') && (f.includes('components/') || f.includes('pages/'))) ||
      files.find(f => f.startsWith('frontend/') && (f.includes('components/') || f.includes('pages/'))) ||
      files.find(f => f.startsWith('client/') && (f.includes('components/') || f.includes('pages/'))) ||
      files.find(f => f.startsWith(workspacePrefix) && f.split('/').pop()![0] === f.split('/').pop()![0].toUpperCase()) ||
      files.find(f => f.startsWith('src/') && f.split('/').pop()![0] === f.split('/').pop()![0].toUpperCase()) ||
      files.find(f => f.startsWith('frontend/') && f.split('/').pop()![0] === f.split('/').pop()![0].toUpperCase()) ||
      files.find(f => f.startsWith('client/') && f.split('/').pop()![0] === f.split('/').pop()![0].toUpperCase()) ||
      files.find(f => f.startsWith(workspacePrefix)) ||
      files.find(f => f.startsWith('src/')) ||
      files.find(f => f.startsWith('frontend/')) ||
